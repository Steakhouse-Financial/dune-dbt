{#
    key notes on yielding stablecoins model:
#}
...
WITH combined_transfers as (
    SELECT * FROM {{ ref('dbt_yielding_evm_stablecoins')}}
)
, all_balance as (
    SELECT blockchain, dt, category, protocol, token_name, token_address, daily_net_amount
